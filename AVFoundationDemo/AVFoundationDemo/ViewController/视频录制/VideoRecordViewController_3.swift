//
//  VideoRecordViewController-2.swift
//  AVFoundationDemo
//
...
/// 视频录制(设置拍摄窗口大小)
class VideoRecordViewController_3: UIViewController {
    ...
    //录制、保存按钮
    var recordButton, saveButton, dismissButton : UIButton!
    ...
    override func viewDidLoad() {
        ...
        // ⚠️⚠️⚠️ 预览窗口是正方形，在屏幕居中（显示的也是摄像头拍摄的中心区域）
        videoLayer.frame = CGRect(x: 0, y: self.view.bounds.height/4, width: self.view.bounds.width, height: self.view.bounds.width)
        ...
        progressBar.backgroundColor = UIColor.white.withAlphaComponent(0.5)
    ...
    func setupButton(){
        ...
        self.dismissButton = UIButton(frame: CGRect(x:0,y:0,width:30,height:30))
        ...
        self.dismissButton.addTarget(self, action: #selector(onClickDismissButton(_:)),
                                     for: .touchUpInside)
        self.dismissButton.layer.position = CGPoint(x: 60,
                                                    y:self.view.bounds.height-50)
        //添加按钮到视图上
    ...
    func mergeVideos() {
        ...
        //⚠️⚠️⚠️ 通过AVMutableVideoComposition实现视频的裁剪(矩形，截取正中心区域视频)
        ...
        let t2 = t1.rotated(by: CGFloat(Double.pi/2))//CGAffineTransformRotate(t1, CGFloat(Double.pi/2))
        ...
        exporter.videoComposition = videoComposition //⚠️⚠️⚠️ 设置videoComposition
...
extension VideoRecordViewController_3: AVCaptureFileOutputRecordingDelegate {
    //录像开始的代理方法
